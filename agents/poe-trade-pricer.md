...
3. **Resolve stat IDs via page JS**, not UI clicking (slow and typo-prone). Run `javascript_tool` **once** in the page context to fetch `/api/trade/data/stats`, find the `explicit` group, then match each item mod's text (numbers replaced with `#`) directly against `entries[].text` using an **exact** match first (`.toLowerCase() === pattern`), falling back to substring only for mods where the exact pattern doesn't hit. Do this in a single pass over the already-fetched data — never re-fetch `/api/trade/data/stats` a second time just to redo the match with a different strategy. Prefer matching against the English pattern; if the item is in Chinese, translate/guess the likely English wording, keep the match keyword short if unsure.
...
5. **Graduated relaxation when `total = 0`.** Don't drop everything at once — relax in stages and tell the user what you dropped at each stage:
   ...
   **Rate limit awareness:** the trade `/search` endpoint has a tight per-window request cap and returns `{"error":{"code":3,"message":"Rate limit exceeded..."}}` when hit, forcing a cold multi-second wait that dwarfs any time saved by parallelizing. Plan the relaxation tiers *before* firing requests (don't probe-and-improvise one at a time), and combine at most 2–3 tier searches into one `javascript_tool` call with sequential `await` between them — never fire a burst of 4+ `/search` POSTs back to back. If a rate-limit error comes back, read the `message` for the wait time and issue one `wait` for that duration rather than retrying immediately or re-issuing the whole batch.

6. **Fetch listing details.** GET `/api/trade/fetch/<ids>?query=<id>` for the result ids (all or top ~10), and extract only `ilvl`, `listing.price`, and `explicitMods.map(m => m.description)` per item — **strip mods down to plain description strings immediately**, don't return the full nested mod objects (tier/level/magnitudes). Returning the full objects routinely blows past the output size limit and truncates mid-JSON, forcing a wasted retry call. Do not extract or report seller account/IGN or whisper info — unnecessary for pricing and an avoidable exposure of other players' data. If a JSON id looks corrupted/truncated in output, refetch that single id on its own. Keep query + fetch in one JS execution and only return the summarized price/mods, not raw ids.
